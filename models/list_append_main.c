...
int main(int argc, char* argv[])
{
    ...
    /* add f1 to the list */
    if (0 != list_push_front(&list, (disposable_t*)f1))
    {
    ...
    /* we should be able to append f5 after f1. */
    if (0 != list_append(&list, list.head, (disposable_t*)f5))
    {
    ...
    MODEL_ASSERT(f1 == list.head->data);
    MODEL_ASSERT(f5 == list.head->next->data);
    MODEL_ASSERT(f5 == list.tail->data);
    MODEL_ASSERT(f1 == list.tail->prev->data);

    /* add f4 to the list. */
    if (0 != list_append(&list, list.head, (disposable_t*)f4))
    {
    ...
    /* append f2 after f1. */
    if (0 != list_append(&list, list.head, (disposable_t*)f2))
    {
        free(f2); free(f3);
        dispose((disposable_t*)&list);
        return 4;
    ...
    /* append f3 after f2. */
    if (0 != list_append(&list, list.head->next, (disposable_t*)f3))
    {
        free(f3);
        dispose((disposable_t*)&list);
        return 4;
