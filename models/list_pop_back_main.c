...
int main(int argc, char* argv[])
{
    ...
    MODEL_ASSERT(f3 == list.head->data);
    MODEL_ASSERT(NULL != list.head->next);
    MODEL_ASSERT(f2 == list.head->next->data);
    MODEL_ASSERT(NULL != list.head->next->next);
    MODEL_ASSERT(f1 == list.head->next->next->data);

    foo_t* fa = NULL;
    foo_t* fb = NULL;
    foo_t* fc = NULL;
    MODEL_ASSERT(3U == list.size);
    MODEL_ASSERT(0 == list_pop_back(&list, (disposable_t**)&fa));
    MODEL_ASSERT(2U == list.size);
    MODEL_ASSERT(0 == list_pop_back(&list, (disposable_t**)&fb));
    MODEL_ASSERT(1U == list.size);
    MODEL_ASSERT(0 == list_pop_back(&list, (disposable_t**)&fc));
